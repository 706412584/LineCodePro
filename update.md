...
- **权限芯片** - 编辑框工具栏新增权限模式芯片，点击循环切换 自动 → 确认 → 只读；修复点击不弹权限选择面板的问题
- **推理档位快捷切换** - ⚡effort 芯片点击弹出 Low/Medium/High/Max 选择底单，当前档位高亮，切换后芯片显示 L/M/H/MAX 短标签
- **顶部布局精简** - 标题栏移除模型胶囊（模型切换保留在编辑区），减少顶部拥挤

### 多发行版 Linux 环境

- **Ubuntu 22.04 支持** - Linux 环境新增发行版选择：Alpine（内置镜像、离线可用）与 Ubuntu 22.04 base（约 28MB，apt 生态），各发行版 rootfs 独立存放、随切随用，已装用户零迁移；Debian 预留待解压器支持 tar.xz 后开放
- **proot 调用加固** - 伪 root（`-0`）、硬链接降级软链（`--link2symlink`，修复 apk/dpkg 在 Android 文件系统上的硬链接失败）、显式绑定 /dev /proc /sys /system；改用 `-r` 而非 `-R` 避免 `-R` 隐式绑定宿主 `/tmp`（ColorOS 上 app uid 不可写导致 apt 建临时文件失败）
- **多镜像源下载** - rootfs 下载按 清华 → 华为云 → 中科大 → 腾讯云 → 官方 逐个回退，单源故障不再阻断安装；Ubuntu 跳过 sha256 强校验（镜像不带 SHA256SUMS，由 gzip CRC + 解压 sanity + probe 兜底）

### API 错误处理与断流重试（cc-haha 机制搬迁）

- **错误分类** - 429 限流 / 529 过载 / 5xx 服务端 / 408 超时 / 连接错误 / 401 认证 / 400 客户端错误分而治之：确定性失败（认证、参数错误）立即终止不再浪费重试；瞬时错误自动重试
- **指数退避** - 500ms×2ⁿ 上限 32s + 随机抖动，服从服务器 Retry-After；请求级上限 10 次、529 过载专项 3 次（此前为固定 3 次 5 秒盲重试）
- **流空闲看门狗** - 流式响应 90 秒无任何数据（含首 token）主动断开重试，不再傻等 10 分钟读超时
- **部分内容保留** - 长回复断流时已流出内容保留为正式回复（此前全部丢弃重来）；一旦响应中出现工具调用（副作用边界）则禁止重发，防止工具重复执行
- **重试提示** - 前 3 次静默，第 4 次起显示倒计时提示；失败文案按错误类型区分（认证失败 / 服务过载 / 通用通信失败）
